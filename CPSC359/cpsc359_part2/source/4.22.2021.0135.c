...
static short int livesCount = 1000;	//Number of lives the player has
...
void* drawThread(void * picture) {
		...
		draw(&(((struct allDrawings*)picture)->score1000place), zeroPtr);
		draw(&(((struct allDrawings*)picture)->score100place), zeroPtr);
		draw(&(((struct allDrawings*)picture)->score10place), zeroPtr);
		draw(&(((struct allDrawings*)picture)->score1place), zeroPtr);
		
		
		draw(&(((struct allDrawings*)picture)->ld), livesPtr);
		draw(&(((struct allDrawings*)picture)->lives10place), zeroPtr);
		draw(&(((struct allDrawings*)picture)->lives1place), zeroPtr);
		
		draw(&(((struct allDrawings*)picture)->md), movesPtr);
		draw(&(((struct allDrawings*)picture)->moves10place), zeroPtr);
		draw(&(((struct allDrawings*)picture)->moves1place), zeroPtr);
...
int main(){
	...
	while(!endFlag) {
		...
		//I don't think I need these anymore? But they'll let
		//add some logic later at least
		if (carCollisionFlag) {
			//printf("On car\n");
		}
